...
class ClosedSessionsQuerier {
	...
	querySessions(email){
		return new Promise((resolve, reject) => {
			this.database.task('Query Closed Sessions', async task => {
	...
	fromPayments(task, sessionID){
		return task.any('SELECT "Payment Capture", "Payment Amount", "Payment Currency" FROM "Payments" WHERE ("Session ID" = $1 AND "Payment Capture" = $2 AND "Payment Status" != $3)', [sessionID, true, "Expired"])
	}
...
module.exports = ClosedSessionsQuerier;
